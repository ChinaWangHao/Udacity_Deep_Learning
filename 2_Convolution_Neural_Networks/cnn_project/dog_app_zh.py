...
#human_face = map(face_detector, human_files_short)
#dog_face = map(face_detector, dog_files_short)

#print('The percentage of human face in human files: {0}%'.format(sum(human_face)))
#print('The percentage of human face in dog files: {0}%'.format(sum(dog_face)))
...
# __回答:__
# 
# 不合理。可以将权重更多的基于头部的轮廓等，可以与dog区分开的人类特征。
...
#dog_in_human = map(dog_detector, human_files_short)
#dog_in_dog = map(dog_detector, dog_files_short)

#print('The percentage of dog in human files: {0}%'.format(sum(dog_in_human)))
#print('The percentage of dog in dog files: {0}%'.format(sum(dog_in_dog)))
...
# 
# __回答:__   
# 1. Convolution层：识别feature.
# 2. Maxpooling层：缩减width和height.
# 3. BatchNormalization层：使神经网络对于超参数的选择上不再那么敏感 并且训练更快.
# 4. Droupout层：防止过拟合.
# 5. GlobalAveragePooling层：减少参数，并使其适用于后续的全连接层.
...
# Conv1: (224, 224, 16) ==> (112, 112, 16)
conv1 = model.add(Conv2D(16, 3, padding='same', input_shape=(224, 224, 3)))
conv1 = model.add(MaxPooling2D(2, 2))
model.add(BatchNormalization())
model.add(Activation('relu'))
conv1 = model.add(Dropout(0.2))

# Conv2: (112, 112, 16) ==> (56, 56, 32)
conv2 = model.add(Conv2D(32, 3, padding='same'))
conv2 = model.add(MaxPooling2D(2, 2))
model.add(BatchNormalization())
model.add(Activation('relu'))
conv2 = model.add(Dropout(0.2))

# Conv3: (56, 56, 32) ==> (28, 28, 64)
conv3 = model.add(Conv2D(64, 3, padding='same'))
conv3 = model.add(MaxPooling2D(2, 2))
model.add(BatchNormalization())
model.add(Activation('relu'))
conv3 = model.add(Dropout(0.2))

# FC1: (28, 28, 64) ==> 133
fc1 = model.add(GlobalAveragePooling2D())
fc1 = model.add(Dense(133, activation='softmax'))
...
model.fit(train_tensors, train_targets, 
          ...
          epochs=epochs, batch_size=32, callbacks=[checkpointer, earlyStopping], verbose=1)
...
VGG16_model.fit(train_VGG16, train_targets, 
          ...
          epochs=50, batch_size=32, callbacks=[checkpointer, earlyStopping], verbose=1)
...
# __回答:__ 
# 步骤：  
# 1. 定义一个Sequential model， 添加一个GlobalAveragePooling层，讲pretraing的数据集作为input data.
# 2. 添加一个全连接层，用softmax作为激活函数，输出nodes为class numbers.  
#   
# 原因：  
# 1. ResNet50很好的解决了深度卷积网络中的梯度递减，可以训练非常深的网络。
# 2. 网络越深，越能抓住更多且更复杂的特征，所以选择了ResNet50.
...
def dog_breed_predcit(img_path):
    bottleneck_feature = extract_Resnet50(path_to_tensor(img_path))
    predicted_vector = Resnet50_model.predict(bottleneck_feature)
    dog_class = dog_names[np.argmax(predicted_vector)]
    return dog_class.split('.')[1]
...
def face_dog_prediction(img_path):
    ...
    if face_detector(img_path):
        return 'Human, looks like' + dog_breed_predcit(img_path)
    
    elif dog_detector(img_path):
        return 'Dog, breed is' + dog_breed_predcit(img_path)
    
    else:
        return 'Error: No human or dog.'
...
#   
# 回答：  
# 1. 输出的结果比预想中好， 预测对了$\frac{5}{6}$的图片。
# 2. 改进想法：  
# （1）通过数据增强的方法，增加数据量以及提高模型robust.  
# （2）尝试不同的预训练网络.  
# （3）增加全连接层的层数，使模型更深.  
# （4）尝试不同的optimize function.
...
img_names = ['dog1.jpg', 'dog2.jpg', 'cat1.jpg', 'cat2.jpeg', 'child1.jpeg', 'man1.jpeg']
...
for i in range(6):
    ...
    ax.set_title(face_dog_prediction(path), fontsize=12)
fig.suptitle('Images classification', fontsize=20)
